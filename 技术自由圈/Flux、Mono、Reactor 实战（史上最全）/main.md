...
![img.png](imgs/excel_calc_demo.png)
...
mono 整体和Flux差不多，只不过这里只会发出0-1个元素。
也就是说不是有就是没有。
...
创建一个包含了从0开始递增的Long对象的序列。其中包含的元素按照指定的间隔来发布。除了间隔时间之外，还可以指定起始元素发布之前的延迟时间 <br>
...
- Spring Cloud Gateway的GlobalFilter和Servlet中的Filter是相似的
